...
app.get("/get-live-test-details", async (req, res) => {
    const get_query = `select * from live_test_details`
    try {
        const result = await db.all(get_query)
        res.send(result)
    } catch (error) {
...
app.post("/get-live-test-start-details", async (req, res) => {
    const {test_id} = req.body
    const get_query = `select * from live_test_details where id = ?`
    try {
        const result = await db.all(get_query, test_id)
        res.send(result)
    } catch (error) {
        console.log(error)
        res.status(400)
...
app.post("/go-live-test", async (req, res) => {
    const { test_id } = req.body
    const get_query = `select * from test_details where id = ${JSON.stringify(test_id)}`

    try {
        const result = await db.get(get_query)
        const {id, type, name, description, minutes, seconds} = result

        const insert_q = `insert into live_test_details values(?, ?, ?, ?, ?, ?)`
        const rm_q = `delete from test_details where id = ?`
        await db.run(insert_q, id, type, name, description, minutes, seconds)
        await db.run(rm_q, id)
        res.send("Success")
    } catch (e) {
        console.log(e)
        res.status(400)
...
app.post("/stop-live-test", async (req, res) => {
    const { test_id } = req.body
    const get_query = `select * from live_test_details where id = ${JSON.stringify(test_id)}`

    try {
        const result = await db.get(get_query)
        const {id, type, name, description, minutes, seconds} = result

        const insert_q = `insert into test_details values(?, ?, ?, ?, ?, ?)`
        const rm_q = `delete from live_test_details where id = ?`
        await db.run(insert_q, id, type, name, description, minutes, seconds)
        await db.run(rm_q, id)
        res.send("Success")
    } catch (e) {
...
app.post("/get-mcq-question-details", async (req, res) => {
    try {
        const { test_id } = req.body
        const get_query = `select * from mcqs where test_id = ${JSON.stringify(test_id)}`
        const result = await db.all(get_query)
...
app.post("/del-test", async (req, res) => {
    const { test_id } = req.body

    try {
        let t_q = `select * from test_details where id = ?`
        const test_details = await db.get(t_q, test_id)
        if (test_details.type === "mcq"){
            let m_q = `select * from mcqs where test_id = ?`
            const mcqs = await db.all(m_q, test_id)
            for (let i = 0; i < mcqs.length; i++) {
                const mcq = mcqs[i]
                await db.run(`delete from options where question_id = ?`, mcq.id)
            }
            await db.run(`delete from mcqs where test_id = ?`, test_id)
            let q = `delete from test_details where id = ?`
            await db.run(q, test_id)
